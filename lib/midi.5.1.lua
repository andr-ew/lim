--modifed from norns/lua for compatibility with Lua 5.1/5.2 and the bit32 library
...
local to_data = {
  ...
  pitchbend = function(msg)
      return {0xe0 + (msg.ch or 1) - 1, bit32.band(msg.val, 0x7f), bit32.band(bit32.rshift(msg.val, 7), 0x7f) }
    end,
...
function Midi.to_msg(data)
  local msg = {}
  -- note on
  if bit32.band(data[1], 0xf0) == 0x90 then
    msg = {
      note = data[2],
      vel = data[3],
      ch = data[1] - 0x90 + 1
  ...
  -- note off
  elseif bit32.band(data[1], 0xf0) == 0x80 then
    msg = {
      type = "note_off",
      note = data[2],
      vel = data[3],
  ...
  -- cc
  elseif bit32.band(data[1], 0xf0) == 0xb0 then
    msg = {
      type = "cc",
      cc = data[2],
      val = data[3],
  ...
  -- pitchbend
  elseif bit32.band(data[1], 0xf0) == 0xe0 then
    msg = {
      type = "pitchbend",
      val = data[2] + bit32.lshift(data[3], 7),
      ch = data[1] - 0xe0 + 1
    }
  -- key pressure
  elseif bit32.band(data[1], 0xf0) == 0xa0 then
    msg = {
      type = "key_pressure",
      note = data[2],
      val = data[3],
  ...
  -- channel pressure
  elseif bit32.band(data[1], 0xf0) == 0xd0 then
    msg = {
